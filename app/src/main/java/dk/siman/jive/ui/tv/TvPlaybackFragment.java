...
import android.support.v17.leanback.widget.PlaybackControlsRow.ThumbsAction;
...
public class TvPlaybackFragment extends android.support.v17.leanback.app.PlaybackOverlayFragment {
    ...
    private void addPlaybackControlsRow(MediaMetadata metadata) {
        ...
        mShuffleAction.setIndex(ShuffleAction.OFF);
        mThumbsUpAction = new ThumbsUpAction(getActivity());

        mPrimaryActionsAdapter.add(mSkipPreviousAction);
        mPrimaryActionsAdapter.add(mPlayPauseAction);
        mPrimaryActionsAdapter.add(mSkipNextAction);
        mPrimaryActionsAdapter.add(mShuffleAction);
    ...
    private void updateAlbumArt(Uri artUri) {
        Bitmap bitmap = ArtHelper.getAlbumArt(getActivity(), artUri);
    ...
    private void updateFavoriteState(boolean change) {
        ...
        LogHelper.i(TAG, "updateFavoriteState");
        ...
        if (musicId != null) {
            //LogHelper.i(TAG, "updateFavoriteState, states: ", isFavorite, " ," , isCurFavorite);
            if (isFavorite && !isCurFavorite) {
                if (change)
                    mMusicProvider.setFavorite(musicId, true);

                //LogHelper.i(TAG, "updateFavoriteState, true");
                mThumbsUpAction.setIndex(ThumbsAction.OUTLINE);
            } else if (!isFavorite && isCurFavorite) {
                if (change) {
                    mMusicProvider.setFavorite(musicId, false);
                    mThumbsUpAction.setIndex(ThumbsAction.SOLID);
                    //LogHelper.i(TAG, "updateFavoriteState, false, 1");
                } else {
                    mThumbsUpAction.setIndex(ThumbsAction.SOLID);
                    //LogHelper.i(TAG, "updateFavoriteState, false, 2");
                }
            } else {
                if (change)
                    mMusicProvider.setFavorite(musicId, false);
